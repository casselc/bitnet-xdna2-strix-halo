...
This index lives on `main` and is the entry point to the work. It is current as of
2026-09-01. SHAs are the branch tips at that date; frozen branches do not move, so
only the two active rows can drift — check with `git ls-remote origin`.

**Paths in the table below are branch-relative and do NOT exist on `main`.** Read
them as "on that branch, at that path" — e.g.
`git show origin/controller-state-envelope:artifacts/controller-state-envelope/RESULTS.md`.
...
| **`controller-state-envelope`** | `60230b5` | **the real multi-domain warm-state envelope**, cache-RAM scaling, thrash, warm open loop, candidate-shape probe | `artifacts/controller-state-envelope/RESULTS.md` |
