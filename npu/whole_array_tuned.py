...
def _build_design(
    ...
    *,
    ...
    fifo_depth = 2
    # C depth is separable from A/B depth: A and B need double buffering to
    # overlap DMA with compute, but C is written once per tile and drained, so
    # depth 1 halves its L1 footprint. The published int8->int32 optimum
    # (96x64x96, 25.31 TOPS) only fits the ~62 KB L1 budget with C single-buffered:
    #     2*m*k + 2*k*n + m*n*sizeof(C) <= 62208
    # The stock depth of 2 is why m=128 and n=128 fail to build.
    c_fifo_depth = int(os.environ.get("C_FIFO_DEPTH", "2"))
    ...
    for col in range(n_aie_cols):
        ...
        # C depth is separable from A/B depth: A and B need double buffering to
        # overlap DMA with compute, but C is written once per tile and drained,
        # so depth 1 halves its L1 footprint and frees room for a larger tile.
    ...
    # Overridable: the C shim DMA's outer repeat stride is

    # m * n_aie_rows * N elements, and the aie.dma_bd stride field is 20 bits

    # (max 1048576). Shrinking the transfer block collapses that repeat

    # dimension, which should remove the oversized stride entirely.

    tb_max_n_rows = int(os.environ.get('TB_MAX_N_ROWS', '0')) or (4 if not c_col_maj else 2)
